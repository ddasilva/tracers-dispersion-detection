...
def main():
    ...
    for aci_file in case_file["ACI_FILES"]:
        ...
        ead_file = find_matching_ead_file(aci_file, case_file["EAD_FILES"])
        if ead_file is None:
            continue

        # Load TRACERS data from disk
        tracers_data = lib_dasilva2026.load_data(aci_file=aci_file, ead_file=ead_file)
        ...
        for _, row in df_match.iterrows():
            lib_dasilva2026.test_detection(
                tracers_data,
                row.start_time,
                row.end_time,
                omni_data,
...
def find_matching_ead_file(aci_file, ead_files):
    """Find the EAD file that matches the given ACI file based on the date token 
    in the filename.
    """
    toks = os.path.basename(aci_file).split("_")
    date_tok = toks[4]
    ead_files = [f for f in ead_files if date_tok in f]

    if len(ead_files) == 0:
        cprint(f"No matching EAD file found for {aci_file}, skipping", "red")
        return None
    elif len(ead_files) != 1:
        cprint(f"Multiple matching EAD files found for {aci_file}, skipping", "red")
        return None
